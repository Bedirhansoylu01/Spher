...
import { apiShareFeed } from './lookup'
...
export function FeedList(props) {
    ...
    useEffect(() => {
        if (sharesDidSet === false){
            ...
            apiShareFeed(handleShareListLookup)
        }
    }, [sharesInit, sharesDidSet, setShareDidSet])//if second argument only is sharesInit consistent loop begin 
    ...
    const handleLoadNext=(event)=>{
        ...
        if (nextUrl !== null){
            ...
            apiShareFeed(handleLoadNextResponse,nextUrl)
        }
    }
