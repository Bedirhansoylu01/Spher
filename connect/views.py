...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def shareactionview(request, commit_id, *args, **kwargs):

    serializer = ShareActionSerializer(data=request.POST)
    if serializer.is_valid(raise_exception=True):
        ...
        qs = share().objects.filter(id=commit_id)
        if not qs.exists():
            return Response({}, status=404)
        obj = qs.first()
        if action == 'like':
